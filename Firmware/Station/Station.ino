...
int tempoutcorrect;      //температура снаружи без шаманства
...
void check_radio(void){
	...
	if (data[1]==ADDRESS)							        //Проверить адрес
		...
		if (farenheit) tempout=(data[2]*1.8)+320;
		else tempout=data[2];                            
    ...
    tempoutcorrect = tempout; //без изменения знака
...
void datasend_to_soprocessor(){
  Wire.write(tempin & 0xFF);
  Wire.write((tempin >>8) & 0xFF);
  Wire.write(humi & 0xFF);
  Wire.write((humi >>8) & 0xFF);
  Wire.write(tempoutcorrect & 0xFF);
  Wire.write((tempoutcorrect >>8) & 0xFF);
...
void setup(){
  ...
  Serial.begin(9600);
  Wire.begin(17);
  Wire.onRequest(datasend_to_soprocessor);
  radio_init();
  pin_set();
  dht.begin();
  delay(300);
